  ...
  toolsData.forEach(tool => {
    ...
    subView.innerHTML = `
      <div class="tool-header-row">
        <button class="tool-back-btn"><i class="fa-solid fa-arrow-left"></i></button>
    ...
    // proper event listener instead of inline onclick
    subView.querySelector(".tool-back-btn").addEventListener("click", closeTool);
  });
...
// make available for the original onclick if any remain
...
function openTool(id) {
  ...
  const intro = document.getElementById("toolbox-intro");
  if (intro) intro.style.display = "none";

  document.querySelectorAll(".toolbox-tool-view").forEach(v => v.classList.remove("active-tool"));

  const target = document.getElementById(`tool-view-${id}`);
  if (target) target.classList.add("active-tool");
}
...
function initBeamCalc() {
  ...
  const calculate = () => {
    ...
    const I = parseFloat(document.getElementById("beam-i").value) * 1e-8;
...
function initMohrCalc() {
  ...
  const drawMohr = (sx, sy, txy, center, R) => {
    ...
    const spanX = Math.max(Math.abs((center + R) - (center - R)), R * 2.5) || 100;
    const spanY = R * 2.5 || 100;
    const scale = Math.min(plotWidth / spanX, plotHeight / spanY);

    const xToPixel = (val) => padding + (val - (center - spanX / 2)) * scale;
  ...
  const calculate = () => {
    ...
    document.getElementById("mc-s1").innerText = (C + R).toFixed(2);
    document.getElementById("mc-s2").innerText = (C - R).toFixed(2);
